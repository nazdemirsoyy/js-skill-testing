...
const categoryArray = getMenuCategories(menu);
...
function loadMenus(menuItems) {
  const displayMenu = menuItems.map(createMenuItem).join("");
  menuListContainer.innerHTML = displayMenu;
}
...
function createFilterButtons() {
  const categoryBtns = categoryArray.map(createButton).join("");
...
function addEventListenerToFilterButtons() {
  ...
  filterBtns.forEach((btn) => {
    btn.addEventListener("click", filterMenus);
  });
}


function filterMenus(event) {
  const category = event.target.id;
  const menuCategory = menu.filter(menuItem => menuItem.category === category);
  const menusToDisplay = category === "All" ? menu : menuCategory;
  loadMenus(menusToDisplay);
}
